...
export function resolveAntigravitySession({ deaddropDir = DEFAULT_DEADDROP_DIR } = {}) {
  // 1. Direct environment variables win (e.g. if invoked within Antigravity itself)
  ...
  // 2. Read session file from <mailbox>/.antigravity_session.json
  const sessionPath = path.join(deaddropDir, SESSION_FILENAME);
  if (!fs.existsSync(sessionPath)) {
    return {
      valid: false,
      error: "missing",
      sessionPath,
      reason: `Antigravity session file not found at ${sessionPath}. Run \'npm run setup-sidecar\' to enable automatic session discovery.`
    };
  }
  ...
  try {
    ...
    return {
      ...
      error: "unreadable",
      sessionPath,
      reason: `Failed to parse Antigravity session file at ${sessionPath}: ${err.message}. Run \'npm run setup-sidecar\' to reconfigure.`
    };
  }

  if (!session?.address || !session?.csrf_token) {
    return {
      valid: false,
      error: "invalid",
      sessionPath,
      reason: `Antigravity session file at ${sessionPath} is missing required \'address\' or \'csrf_token\' fields. Run \'npm run setup-sidecar\' to reconfigure.`
    };
  }
  ...
  if (session.pid && !isPidAlive(session.pid)) {
    return {
      ...
      sessionPath,
      pid: session.pid,
      reason: `Antigravity session at ${sessionPath} is stale (PID ${session.pid} is not running). Run \'npm run setup-sidecar\' if the sidecar is not auto-starting.`
    };
  }
...
function wakeViaAgentapi({ config, agentConfig, to, from, filename, deaddropDir = DEFAULT_DEADDROP_DIR }) {
  ...
  const sessionResult = resolveAntigravitySession({ deaddropDir });
  if (sessionResult.valid === false) {
    console.error(`[Dead Drop] Note: ${sessionResult.reason}`);
    return Promise.resolve({ woke: false, reason: sessionResult.reason });
  }
  ...
  const env = {
    ...process.env,
    ANTIGRAVITY_LS_ADDRESS: sessionResult.address,
    ANTIGRAVITY_CSRF_TOKEN: sessionResult.csrfToken,
  };

  return new Promise((resolve) => {
    execFile(
      agentapiPath,
      ["send-message", "--title=Dead Drop Mail", conversationId.trim(), wakeText],
      { env },
      (error, stdout, stderr) => {
        if (error) {
...
export async function wakeAgentIfTargeted({ to, from, filename, deaddropDir = DEFAULT_DEADDROP_DIR }) {
  ...
  return handler({ config, agentConfig, to, from, filename, deaddropDir });
}
